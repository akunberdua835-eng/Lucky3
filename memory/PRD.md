...
## Implemented- 2026-02-21: MVP auth, batch, owners, transaksi, generator, analytics, excel, voucher.
...
## Catatan teknis
- 2026-09-02 (fase 4 — PPh 22 dikelola): PPh 22 tidak lagi hardcode 0,25%. Ada tarif default global di collection `settings` (`GET/PUT /api/settings/pph`, PUT khusus admin), override per tanggal (`POST /api/finance/pph` mode `auto` dengan `rate_pct` atau mode `manual` dengan `amount` + `note`), dan reset (`DELETE /api/finance/pph/{date}?batch_id=`). `finance_summary` memakai `calc_pph()` dan mengembalikan `pph_mode`, `pph_rate_pct`, `pph_custom`, `pph_note` per hari + `pph_rate_default`, `pph_effective_pct`, `pph_manual_days` di totals. Untung/rugi = harga_jual − pph22 − total_modal. Excel laba rugi menambah kolom "Tarif / Mode PPh". UI: kartu "Pengaturan PPh 22" (admin) + modal "Kelola PPh 22" dengan preview untung/rugi. Health check publik `GET /api/`.
- 2026-09-02 (deploy mandiri): disiapkan `frontend/vercel.json`, `backend/Dockerfile`, `backend/Procfile`, `backend/runtime.txt`, `render.yaml`, `railway.json`, `backend/.env.example`, `frontend/.env.example`, `backend/scripts/migrate_to_atlas.py`, dan `DEPLOY.md` (panduan Vercel + Railway/Render + MongoDB Atlas M0). Backend di Vercel opsional lewat `backend/vercel.json` + `backend/api/index.py` (tidak direkomendasikan: cold start & timeout 10s).
- 2026-09-02 (fase 3): laporan Excel dipindah ke `backend/excel_reports.py` (judul, header hijau, border, number_format Rp/Kg, rumus =SUM, grafik line untuk harian & harga). Endpoint: `/api/export/excel` (multi-sheet lengkap), `/api/export/owners-excel` (+`?owner=` -> 1 sheet nama orang), `/api/export/daily-excel`, `/api/export/finance-excel`, `/api/export/prices-excel?month=`. Harga/Kg: `GET /api/prices?month=` (change, change_pct, trend, stats), `POST /api/prices` (date/price_per_kg/note), `DELETE /api/prices/{date}` — hanya referensi/monitoring, tidak mengubah transaksi. Frontend: tab "Harga / Kg" (grid tanggal 1–31, metrik, grafik tren, CRUD, export), tabel "Transaksi terakhir" + rekap pemilik/tanggal kini scrollable (sticky header). Admin utama = yumaclovstar@gmail.com (`ADMIN_EMAIL`), admin lama admin@ptsjm.co.id disimpan sebagai cadangan via `BACKUP_ADMIN_EMAILS` dan dilindungi dari penghapusan.
- 2026-09-02 (restore dari GitHub akunberdua835-eng/Lucky2 commit 148a7cc): seluruh kode disinkronkan ke /app, dependencies backend (+openpyxl) & frontend diinstall, `.env` backend dibuat ulang (ADMIN_EMAIL/ADMIN_PASSWORD/JWT_SECRET/CORS_ORIGINS eksplisit) karena .env tidak ikut ter-commit. Kredensial admin baru ada di /app/memory/test_credentials.md. Pytest: 47/48 lulus (1 gagal karena `tests/test_finance_iter3.py` meng-hardcode LIVE_BATCH=BATCH-2026-08 yang tidak ada di DB baru — bukan bug kode). Email Resend tetap MOCKED.
